 ...
 *
 * Light mode: the header row sits on the pale well tone and every rule is
 * navy at low alpha, never grey — see ui/surfaces.ts for why.
 */
...
export const HEAD_ROW = 'border-b border-[#1e3a8a]/[0.14] dark:border-sky-400/15';

/** Every header cell, pinned or not. Sticky to the top of the scroll box. */
export const HEAD_CELL =
  'sticky top-0 z-20 bg-[#f5f8fd] dark:bg-[#0b1324] select-none py-3 text-left align-middle font-semibold uppercase tracking-wide text-[11px] leading-tight text-slate-600 dark:text-slate-400';
...
export const ROW = 'group/row border-b border-[#1e3a8a]/[0.08] dark:border-sky-400/[0.08] transition-colors';

export const rowTint = (selected: boolean) =>
  selected
    ? 'bg-blue-500/5 dark:bg-blue-500/10 hover:bg-blue-500/10 dark:hover:bg-blue-500/[0.14]'
    : 'hover:bg-[#f3f6fc] dark:hover:bg-sky-400/[0.05]';

/** Body cell with the thin column rule on its right. */
export const CELL =
  'px-3 py-3 align-middle border-r border-[#1e3a8a]/[0.06] dark:border-sky-400/[0.07] text-slate-700 dark:text-slate-300';

/** The rule between the pinned block and the scrolling columns is a touch
 *  stronger so the boundary reads while scrolling. */
export const CELL_PINNED_EDGE = 'border-r border-[#1e3a8a]/[0.16] dark:border-sky-400/20';
...
export const pinnedBg = (selected: boolean) =>
  selected
    ...
    : 'bg-white group-hover/row:bg-[#f3f6fc] dark:bg-[#0b1324] dark:group-hover/row:bg-[#111c33]';
