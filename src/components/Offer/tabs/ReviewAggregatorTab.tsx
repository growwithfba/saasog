...
import { Upload, Sparkles, Loader2, CheckCircle, AlertCircle, Plus, FileText, MessageSquare, Lightbulb, HelpCircle } from 'lucide-react';
...
export function ReviewAggregatorTab({ productId, data, onChange }: ReviewAggregatorTabProps) {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && (file.type === 'text/csv' || file.name.endsWith('.csv') || file.name.endsWith('.doc') || file.name.endsWith('.docx'))) {
      setSelectedFile(file);
      setError(null);
      setSuccess(false);
    } else {
      setError('Please select a valid CSV or document file');
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Review Uploader Section - Enhanced */}
      {!hasReviewData && (
