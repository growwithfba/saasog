...
export function VettingDetailContent({ asin }: { asin: string }) {
  ...
  const copyShareUrl = async () => {
    ...
    try {
      ...
      setShareToast({ kind: 'error', message: 'Could not copy — open the share modal to copy manually.' });
    }
  };

  const handleShareClick = async () => {
    if (!submission?.id || shareBusy) return;
    // If not currently shared, turn sharing on first.
      ...
      {shareToast && (
        <div className="fixed bottom-4 right-4 z-[200]">
          ...
          >
            ...
            <p className="font-medium text-sm">{shareToast.message}</p>
          </div>
        </div>
      )}
    </div>
